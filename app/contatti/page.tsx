import { Metadata } from "next";
import { ContactForm } from "@/components/sections/ContactForm";

export const metadata: Metadata = {
  title: "Contatti | Mundo Gin",
  description:
    "Per scoprire di più su Mundo Gin, collaborazioni o richieste speciali. Contattaci.",
};

export default function ContattiPage() {
  return (
    <div>
      {/* Hero: immagine a tutta pagina con titolo e sottotitolo in basso a sinistra */}
      <section className="relative h-screen min-h-[480px] w-full">
        <img
          src="/images/biglietti.png"
          alt=""
          aria-hidden
          className="absolute inset-0 w-full h-full object-cover object-center"
        />
        <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 via-black/30 to-transparent pt-24 pb-12 sm:pb-16 px-4 sm:px-6 lg:px-8">
          <div className="container mx-auto max-w-7xl">
            <h1 className="text-mundo-white font-futura-500 font-medium uppercase mb-3" style={{ fontSize: "60px", lineHeight: "69px" }}>
              Contatti
            </h1>
            <p className="font-futura-400 text-mundo-white/90 text-lg sm:text-xl max-w-2xl">
              Per scoprire di più su Mundo Gin, collaborazioni o richieste speciali, non esitare a contattarci. Siamo qui per rispondere alle tue domande.
            </p>
          </div>
        </div>
      </section>

      {/* Company Profile + Form */}
      <section className="py-16 sm:py-20 lg:py-32 bg-[#F2F2F2]">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-start">
            {/* Company Profile */}
            <div>
              <h2 className="text-mundo-black font-futura-500 font-medium text-2xl sm:text-3xl mb-8">
                Company Profile
              </h2>
              <hr className="border-mundo-black/20 mb-6" />
              <a
                href="mailto:info@mundogin.com"
                className="font-futura-400 text-mundo-black hover:text-mundo-black/80 transition-colors block mb-6"
              >
                info@mundogin.com
              </a>
              <hr className="border-mundo-black/20 mb-6" />
              <p className="font-futura-400 text-mundo-black/80 mb-6">
                Via Leonardo Da Vinci 37, 24043
                <br />
                Caravaggio (BG)
              </p>
              <hr className="border-mundo-black/20 mb-6" />
              <ul className="flex flex-wrap gap-4 font-futura-400 text-sm">
                <li>
                  <a
                    href="https://www.instagram.com/mundodrygin/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-mundo-black hover:text-mundo-black/80 transition-colors"
                    aria-label="Instagram"
                  >
                    <span className="w-6 h-6 shrink-0" aria-hidden>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                        <path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" />
                      </svg>
                    </span>
                    Instagram
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.facebook.com/people/Mundo-Gin/61558116593022/"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-mundo-black hover:text-mundo-black/80 transition-colors"
                    aria-label="Facebook"
                  >
                    <span className="w-6 h-6 shrink-0" aria-hidden>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                        <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                      </svg>
                    </span>
                    Facebook
                  </a>
                </li>
                <li>
                  <a
                    href="https://www.tiktok.com/@mundogin"
                    target="_blank"
                    rel="noopener noreferrer"
                    className="inline-flex items-center gap-2 text-mundo-black hover:text-mundo-black/80 transition-colors"
                    aria-label="TikTok"
                  >
                    <span className="w-6 h-6 shrink-0" aria-hidden>
                      <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" className="w-6 h-6">
                        <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 20.1a6.34 6.34 0 0 0 10.86-4.43v-7a8.16 8.16 0 0 0 4.77 1.52v-3.4a4.85 4.85 0 0 1-1-.1z" />
                      </svg>
                    </span>
                    TikTok
                  </a>
                </li>
              </ul>
            </div>

            {/* Form */}
            <div className="bg-mundo-white rounded-lg p-6 sm:p-8 shadow-sm">
              <ContactForm />
            </div>
          </div>
        </div>
      </section>
    </div>
  );
}
